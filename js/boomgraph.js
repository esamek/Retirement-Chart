// ┌────────────────────────────────────────────────────────────────────┐ \\
// │ HelloWallet Special Tooltips                                       │ \\
// ├────────────────────────────────────────────────────────────────────┤ \\
(function($){

    $.HW = $.HW || {};

    function chartTooltip(plot,data,x,y){
        return this.init(plot,data,x,y);
    ...
    var methods = {
        ...
        init: function(plot,data,x,y){

            this.plot = plot;
            this.plotType = this.plot.type;
            this.dataName = data.series[x].name;
            this.dataValue = data.series[x].data[y];
            this.chart = {};
            this.chart.$container = $(plot.paper.canvas.parentElement);
            this.chart.offsetX = this.chart.$container.offset().top;
            this.chart.offsetY = this.chart.$container.offset().left;
            this.x = this.plotType == "circle" ? this.plot.attr('cx') : this.plot.attr('x') + this.plot.attr('width') / 2;
            this.y = this.plotType == "circle" ? this.plot.attr('cy') : this.plot.attr('y');
        ...
        createTooltipDOM: function(){
            ...
            $t.css(css);
            $t.html('<label>' + that.dataValue + '<br /><small>' + that.dataName + '</small></label>');
            $t.addClass('chart-tooltip');
            this.chart.$container.css('position','relative');

            this.chart.$container.append($t);
            this.TOOLTIP = $t;
            return this;
        },
        ...
        show: function(){
            var that =this;
            var X = that.x - (that.TOOLTIP.outerWidth() / 2);
            var Y = that.y - that.TOOLTIP.outerHeight() - 10;

            this.TOOLTIP.stop(true,true).show().animate({
                'opacity': 1,
                'top': Y,
                'left': X
            }, 500);
    ...
    chartTooltip.prototype = $.extend(true,{},chartTooltip.prototype,methods);

    $.HW.chartTooltip = function(plot,data,x,y){
        return new chartTooltip(plot,data,x,y);
    };
})(jQuery);


// ┌────────────────────────────────────────────────────────────────────┐ \\
// │ END HelloWallet Special Tooltips                                       │ \\
// ├────────────────────────────────────────────────────────────────────┤ \\
...
var Choopy = (function(){
                ...
                for(var i= 0; i < trs.length; i++) {
                    ...
                    if(tr.getElementsByTagName('th')){
                        ...
                        for(var j=0,jj=tds.length;j<jj;j++){
                            tdsValues.push(this.utils.trim($(tds[j]).text()));
                        }
                    }
        ...
        if(this.options.grid.draw.y){
            ...
            for (var i = 0; i <= this.draw.coord.scale.y.tickers; i++) {
                if(i < this.draw.coord.scale.y.tickers){
                    var line = this.draw.paper.path(["M", x, Math.round(y + i * rowHeight) + .5, "l", w, 0]).attr({
                        fill: "none",
                        stroke: this.options.color.grid,
                        "stroke-dasharray": "- ",
                        opacity: 0.3
    ...
    Choopy.prototype.drawLine = function(idSerie,color,howToScale){
        ...
        for (var j = 0, jj = values.length; j < jj ; j++){
            ...
            this.draw.tooltips[dot.id] = $.HW.chartTooltip(dot,self.data,idSerie,j);
    ...
    Choopy.prototype.drawColumns = function(idSerie,color,howToScale){
        ...
        for (var j=0,jj=values.length; j<jj ; j++){
            ...
            this.draw.tooltips[rect.id]= $.HW.chartTooltip(rect,self.data,idSerie,j);
    ...
    Choopy.prototype.drawTooltipForPlot = function(plot,x,y){
        var ppp,textLabel,isFunc = this.utils.isFunction;


        var title=this.draw.paper.text(0, 0, "").attr({
            'text':this.options.tooltip.renderer(this.data,x,y).title+'\n'+this.options.tooltip.renderer(this.data,x,y).sub
        ...
        if (plot.attrs.cx + title.getBBox().width > this.options.width) {
            side = "left";
            xPPP-=8
        }
        ppp = this.draw.paper.popup(xPPP,yPPP ,title,side,true);

        var tooltip=this.draw.paper.path().attr({
            path:ppp.path,
            fill: this.options.tooltip.style.fill || "#000",
            stroke: this.options.tooltip.style.stroke || "#999",
            "stroke-width": this.options.tooltip.style.strokeWidth || 1,
            "fill-opacity": this.options.tooltip.style.fillOpacity || .7
        }).transform("T" + ppp.dx + "," + ppp.dy);

        title.attr({'x': ppp.dx, 'y': ppp.dy - 10}).toFront();

        var ToolTip = this.draw.paper.set(tooltip,title);

        if(this.options.events.tooltip.init && isFunc(this.options.events.tooltip.init)){
            return this.options.events.tooltip.init(ToolTip).hide();
        }else{
            return ToolTip.hide();
        }
    ...
    //        var titleF=this.draw.paper.text(160, 10, '').attr({'text':this.options.tooltip.renderer(this.data,x,y).title}).attr(this.options.textes.tooltip.title)
    //        var subtitleF=this.draw.paper.text(160, 27, '').attr({'text':this.options.tooltip.renderer(this.data,x,y).sub}).attr(this.options.textes.tooltip.sub)
    ...
    Choopy.prototype.drawTooltipForColumn = function(column,x,y){
        ...
        if (column.attrs.cy - title.getBBox().height <0) {
            side = "bottom";
            xPPP+=2;
        }
        ppp = this.draw.paper.popup(xPPP,yPPP ,title,side,true);
    ...
    //        textLabel.push(this.draw.paper.text(60, 12, this.options.tooltip.renderer(this.data,x,y).title).attr(this.options.textes.tooltip.title))
    //        textLabel.push(this.draw.paper.text(60, 27, this.options.tooltip.renderer(this.data,x,y).sub).attr(this.options.textes.tooltip.sub))
    //
    //        var side = "top";
    //
    //        var xPPP=column.attrs.x+column.attrs.width/2,
    ...
    Choopy.prototype.hover = function(elem){
        var self = this,
            isFunc = this.utils.isFunction;
        elem.hover(function(){
            var el = this;
            if(this.type == 'circle'){

                if(self.options.events.dot.over && isFunc(self.options.events.dot.over)){
                    self.options.events.dot.over(this);
                }else{
            ...
            if(this.type == 'rect'){
                if(self.options.events.bar.over && isFunc(self.options.events.bar.over)){
                    self.options.events.bar.over(this);
                }else{
                    this.attr('fill-opacity', 0.7);
                }
            }

            self.draw.tooltips[this.id].show();

        },function(){

            if(this.type == 'circle'){
                if(self.options.events.dot.out && isFunc(self.options.events.dot.out)){
                    self.options.events.dot.out(this);
                }else{
            ...
            if(this.type == 'rect'){
                if(self.options.events.bar.out && isFunc(self.options.events.bar.out)){
                    self.options.events.bar.out(this);
                }else{
    ...
    Choopy.prototype.defaultOptions = {
            ...
            y:{
                ...
                labels:{
                    ...
                    render:function(string){    // if you want to add a units or something else, it's here
                        return {
                            font: '10px "Proxima Nova Condensed"',
                            fill: "#333",
                            'font-weight': 600,
                            text: string.toString().toUpperCase()
                        };
                    }
                },
                legend:{
        ...
        textes:{
            axis:{
                ...
                x:{
                    ...
                    legend:{
                        show:true,
                        font: '12px "Proxima Nova Condensed"',
                        fill: "#333"
                    }
                }
            },
        ...
        tooltip:{
            renderer: function(data,x,y){             // customize the value shown on your tooltips
                return {
        ...
        color :{
            serie:["#ea4648","#f5871e","#de1c85","#f9c031","#004b77","#00b2e9"],
            transverse:"green",
            dot:{
                fill:"#EEE"
            },
